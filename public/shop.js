...
async function populateShop(){
  ...
  function renderResults(){
    ...
    const html = page.map(p=>`<a class="card wrap" href="/product.html?id=${p.id}">
      ...
      <img src="${(p.images&&p.images[0])||'/public/images/placeholder.svg'}" alt="${p.name}"><div class="pad"><div>${p.name}</div><div class="muted">${(p.category||'').toString().replace(/^./,c=>c.toUpperCase())}</div><div class="price">$${p.price.toFixed(2)}</div></div>
